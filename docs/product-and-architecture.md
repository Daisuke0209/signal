...
測定する。利用者から API キーの利用許可を得たため実接続も検証する。
モック検証、実 API の応答、実際の Meet 音声での確認を区別して記録する。
...
## 決定 7: 実際の会話に紐づく返答と累積確認リスト

2026-09-06 の音声入力確認後のフィードバックを反映する。返答例には、その返答が
対象とする顧客の発言を表示する。モデルは入力にある発言IDを選び、サーバーが
同じ会話・顧客側・入力時点内であることを検証して本文を取得する。質問をモデルに
自由記述させる方法より、発言の取り違えや存在しない質問の表示を抑えられる。

確認事項は提案runごとの一時的な配列から、会話ごとの永続チェックリストへ移す。
「次に聞くこと」はこのリストへ統合し、未確認の項目を質問の手がかりにする。
生成のたびに置き換えず、既存項目を維持しながら追加する。明示的に確認できた発言を
根拠に自動でチェックし、その発言も表示する。これはメール送信などの外部操作ではなく
会話整理の状態更新なので承認は挟まない。誤判定を訂正できる手動操作と競合versionを
用意し、人の訂正は後の自動生成より優先する。

ブラウザだけで状態を保存する方法は単純だが、再読み込みや別端末で状態を失うため採らない。
表示は提案更新時にAPIから取得し、他画面での手動訂正は表示中のみ5秒間隔で再確認する。
取得失敗中も保存済み項目を維持し、アクセス失効時は表示を消す。終了した商談は閲覧専用。
モデルの判定は誤り得るため、チェックの根拠表示と訂正手段を省略しない。

文字起こしは末尾を見ている間だけ、新しい確定発言・途中の書き起こしに追従する。
過去の発言を読んでいる間は位置を維持し、「最新へ戻る」で追従を再開する。
