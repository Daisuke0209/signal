...
初期版は Mac Chrome の Google Meet 音声と事前登録 PDF を使い、自動で営業支援を行います。
画面構成、責任分担、設計の理由と受け入れ条件は[プロダクト・設計方針](docs/product-and-architecture.md)を参照してください。
各機能は小さな Issue/PR 単位で実装しており、方針書の全機能が実装済みという意味ではありません。
